...
// import DynamicInteractiveLayers from './DynamicInteractiveLayers';
...
const J40Map = ({ location }: IJ40Interface) => {
  ...
  const onClick = (event: MapEvent | React.MouseEvent<HTMLButtonElement>) => {
    ...
    // even.target = property that refers to teh DOM element that was //
    // clicked. Allows you to determine what part of the map the user
    // interacted with
    // The event.target is cast to an HTMLElement using TypeScript's as keyword.
    // This is necessary because event target is of type Event Target
    // which is a more generic type and does not have properties like id
    // or nodeName.
    // By casting it to HTMLElement, you can safely access these
    // properties.
  ...
  //   constants.PRIORITIZED_HIGH_ZOOM_LAYER_ID, // Default to the grouped layers
  // ]);
    ...
    <>
      <Grid desktop={{ col: 9 }} className={styles.j40Map}>
        ...
        <ReactMapGL
          ...
          touchRotate={false}
          // eslint-disable-next-line max-len
          // ADDED NEW LAYERS HERE!!!
          interactiveLayerIds={[
            constants.HIGH_ZOOM_LAYER_ID,
            constants.PRIORITIZED_HIGH_ZOOM_LAYER_ID,
            //   constants.PSIM_BURDEN_HIGH_LAYER_ID,
            //   constants.ADD_BURDEN_HIGH_LAYER_ID,
          ]}
          // ****** Callback props: ******
        ...
        >
          ...
          {/* Pass visibleLayers and setVisibleLayers to MapTractLayers */}
          {/* <MapTractLayers
            selectedFeature={selectedFeature}
            selectedFeatureId={selectedFeatureId}
            visibleLayers={visibleLayers}
          ...
          {/* Pass visibleLayers and setVisibleLayers to DynamicInteractiveLayers */}
          {/* <DynamicInteractiveLayers
            visibleLayers={visibleLayers}
            setVisibleLayers={setVisibleLayers}
...
const J40Map = ({ location }: IJ40Interface) => {
  ...
  console.log("Interactive layer IDs:", interactiveLayerIds);
  /**
   * Selects the provided feature on the map.
   * @param feature the feature to select
   */
